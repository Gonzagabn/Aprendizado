...
-- OU
SELECT NAME FROM v$database; /*AQUI É O NOME DO SERVIÇO*/

/*NO ORACLE, DIFERENTE DO MYSQL E DO SQL SERVER, NÃO FAZ SENTIDO CHAMAR UM BANCO, PORQUE 
TUDO ESTÁ UM UMA UNICA INSTANCIA, OU ÚNICO DATABASE. O MAIS PRÓXIMO DISSO, AS VEZES PODE
SER FEITO É TROCAR O SCHEMA*/

ALTER SESSION SET current_schema = other_user;

/*MOSTRAR AS TABELAS DE UM BANCO DE DADOS*/

--AQUI MOSTRANDO APENAS DO USUARIO ATUAL
SELECT table_name
FROM user_tables
ORDER BY table_name;
--OU, AQUI MOSTRANDO TODAS AS TABELAS QUE O USUÁRIO ATUAL TEM PERMISSÃO PRA VER
SELECT table_name
FROM all_tables
ORDER BY table_name;
--OU AINDA, MOSTRAR TODAS EXISTENTES, MAS REQUER PRIVILEGIOS
SELECT table_name 
FROM dba_tables;

/*DESCREVER UMA TABELA*/

SELECT column_name AS "NAME", nullable AS "Null?", 
